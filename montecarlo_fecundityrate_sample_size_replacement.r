...
sampmontc <- seq(from=10,to=120,by=5)   # resample levels. It is now a sequence from 20 to 100, in increments of 5 seals
...
for (y in unique(ovar$year)){
  for (smontc in sampmontc){
    for (j in 1:nmontc){
      ...
      montcdat <- data.frame(ID=sample(dat$ID,smontc,replace=TRUE))
